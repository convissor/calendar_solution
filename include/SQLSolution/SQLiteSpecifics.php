 ...
 * @version    $Name:  $ $Id: sql-lite.inc,v 5.11 2009-12-24 23:35:21 danielc Exp $
...
class SQLSolution_SQLiteSpecifics extends SQLSolution_Customizations {
    ...
    function Connect($FileName, $FileLine) {
        ...
        $this->SQLConnection = @sqlite_open($this->SQLDbName,
                $this->SQLPermissions) or die
    ...
    function PersistentConnect($FileName, $FileLine) {
        ...
        $this->SQLConnection = @sqlite_popen($this->SQLDbName,
                $this->SQLPermissions) or die
                ($this->KillQuery($FileName, $FileLine, $php_errormsg));
    }
    ...
    function RunQuery($FileName, $FileLine) {
        ...
        if ($this->SQLRecordSet = @sqlite_query($this->SQLConnection,
                $this->SQLQueryString)) {
    ...
    function FieldName($FileName, $FileLine, $FieldNumber) {
        ...
        if ($Output = @sqlite_field_name($this->SQLRecordSet, $FieldNumber)) {
            return $Output;
        } else {
            $this->KillQuery($FileName, $FileLine, $php_errormsg);
        }
    ...
    function RecordAsAssocArray($FileName, $FileLine, $SkipSafeMarkup = array()) {
        ...
        if ($Temp = @sqlite_fetch_array($this->SQLRecordSet, SQLITE_ASSOC)) {
    ...
    function RecordAsEnumArray($FileName, $FileLine, $SkipSafeMarkup = array()) {
        ...
        if ($Temp = @sqlite_fetch_array($this->SQLRecordSet, SQLITE_NUM)) {
            foreach ($Temp as $Key => $Val) {
                if ($this->SQLEscapeHTML != 'N') {
                    $Val = htmlspecialchars($Val);
                }
    ...
    function InsertID($FileName, $FileLine, $Table = '', $Field = '', $Where = '',
        ...
        if ($Output = @sqlite_last_insert_rowid($this->SQLConnection)) {
            return $Output;
        } elseif ($php_errormsg == '') {
